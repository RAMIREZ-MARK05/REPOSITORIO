...
// 04.10.2015 - 14.08.2023
// Dateiname resonance0.js verwendet, da Konflikt mit einer Datei namens resonance.js beobachtet
...
function start () {
  ...
  ipD.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Eingabe Federkonstante)
  ipM.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Eingabe Masse)
  ipA.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Eingabe Dämpfung)
  ipF.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Eingabe Kreisfrequenz)
...
function input () {
  ...
  if (ae == ipD) focus(ipM);                               // Fokus für nächstes Eingabefeld
  if (ae == ipM) focus(ipA);                               // Fokus für nächstes Eingabefeld
  if (ae == ipA) focus(ipF);                               // Fokus für nächstes Eingabefeld
  if (ae == ipF) ipF.blur();                               // Fokus abgeben
  }
