...
function start () {
  ...
  focus(ipX0);                                             // Fokus für erstes Eingabefeld
  ...
  ipX0.onblur = reaction;
  ipV0.onblur = reaction;
  ipA.onblur = reaction;
...
// Seiteneffekt bu2.state, t, on, timer, slow, x0, v0, a, tLB1, tLB2, Wirkung auf Eingabefelder und Zeichenfläche
...
function reactionReset () {
  ...
  reaction();                                              // Eingegebene Werte übernehmen, rechnen und neu zeichnen
  focus(ipX0);                                             // Fokus für Eingabefeld
  }
...
// Seiteneffekt x0, v0, a, tLB1, tLB2, Wirkung auf Eingabefelder und Zeichenfläche
...
function reactionMove (u, v) {
  if (nrLB == 0) return;                                   // Abbrechen, falls keine Lichtschranke ausgewählt
  var re = canvas.getBoundingClientRect();                 // Lage der Zeichenfläche bezüglich Viewport 
  u = Math.round(u-re.left);                               // Waagrechte Koordinate bezüglich Zeichenfläche
  v = Math.round(v-re.top);                                // Senkrechte Koordinate bezüglich Zeichenfläche
...
function timeLB (xLB) {
  ...
  if (t1 < 0) return -1;                                   // Rückgabewert, falls beide Lösungen negativ
  if (t2 < 0) return t1;                                   // Rückgabewert, falls nur kleinere Lösung negativ
  return t2;                                               // Rückgabewert, falls keine Lösung negativ
  }
...
function input () {
  ...
  if (ae == ipX0) focus(ipV0);                            // Fokus für nächstes Eingabefeld
  if (ae == ipV0) focus(ipA);                             // Fokus für nächstes Eingabefeld
  if (ae == ipA) ipA.blur();                              // Fokus abgeben
  }
