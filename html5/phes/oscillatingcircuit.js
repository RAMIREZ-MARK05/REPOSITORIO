...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel oscillatingcircuit_de.js) abgespeichert.
...
var rb1, rb2;                                              // Obere Radiobuttons (Zeitlupe)
var ip1;                                                   // Erstes Eingabefeld (Kapazität)
var ip2;                                                   // Zweites Eingabefeld (Induktivität)
var ip3;                                                   // Drittes Eingabefeld (Widerstand)
var ip4;                                                   // Viertes Eingabefeld (Maximalspannung)
var rb3, rb4;                                              // Untere Radiobuttons (Diagramme)
...
function start () {
  ...
  getElement("rb1lb",text03);                              // Zugehöriger Text
  rb2 = getElement("rb2");                                 // Radiobutton (Zeitlupe 100 x)
  getElement("rb2lb",text04);                              // Zugehöriger Text
  getElement("ip1a",text05);                               // Erklärender Text (Kapazität)
  ip1 = getElement("ip1b");                                // Eingabefeld für Kapazität
  getElement("ip1c",microfarad);                           // Einheit (Mikrofarad)
  getElement("ip2a",text06);                               // Erklärender Text (Induktivität)
  ip2 = getElement("ip2b");                                // Eingabefeld für Induktivität
  getElement("ip2c",henry);                                // Einheit (Henry)
  getElement("ip3a",text07);                               // Erklärender Text (Widerstand)
  ip3 = getElement("ip3b");                                // Eingabefeld für Widerstand
  getElement("ip3c",ohm);                                  // Einheit (Ohm)
  getElement("ip4a",text08);                               // Erklärender Text (Maximalspannung)
  ip4 = getElement("ip4b");                                // Eingabefeld für Maximalspannung
  getElement("ip4c",volt);                                 // Einheit (Volt)
  rb3 = getElement("rb3","");                              // Radiobutton (Spannung/Stromstärke)
  rb3.checked = true;                                      // Radiobutton ausgewählt
  getElement("rb3lb",text09);                              // Zugehöriger Text
  rb4 = getElement("rb4","");                              // Radiobutton (Energie)
  getElement("rb4lb",text10);                              // Zugehöriger Text
  ...
  ip1.onkeydown = reactionEnter;                           // Reaktion auf Enter-Taste (Eingabe Kapazität)
  ip2.onkeydown = reactionEnter;                           // Reaktion auf Enter-Taste (Eingabe Induktivität)
  ip3.onkeydown = reactionEnter;                           // Reaktion auf Enter-Taste (Eingabe Widerstand)
  ip4.onkeydown = reactionEnter;                           // Reaktion auf Enter-Taste (Eingabe Maximalspannung)
  ip1.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Eingabe Kapazität)
  ip2.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Eingabe Induktivität)
  ip3.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Eingabe Widerstand)
  ip4.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Eingabe Maximalspannung)
...
function enableInput (p) {
  ip1.readOnly = !p;                                       // Eingabefeld für Kapazität
  ip2.readOnly = !p;                                       // Eingabefeld für Induktivität
  ip3.readOnly = !p;                                       // Eingabefeld für Widerstand
  ip4.readOnly = !p;                                       // Eingabefeld für Maximalspannung
  }
  
// Reaktion auf Resetknopf:
// Seiteneffekt bu2, on, t, t0, tD, cap, ind, res, voltage, voltage0, amperage, amperage0, ip1, ip2, ip3, ip4, 
...
// Seiteneffekt t0, bu2, cap, ind, res, voltage0, ip1, ip2, ip3, ip4, alpha, omega2, omega, c1, c2, beta1, beta2, 
...
// Seiteneffekt cap, ind, res, voltage0, ip1, ip2, ip3, ip4, alpha, omega2, omega, c1, c2, beta1, beta2, 
// d1, d2, d3, d4, T, u0exp, sin, cos, amperage0
...
function input () {
  ...
  cap = inputNumber(ip1,0,true,minCapMy,maxCapMy)/1e6;     // Kapazität (F)
  ind = inputNumber(ip2,2,true,minInd,maxInd);             // Induktivität (H)
  res = inputNumber(ip3,1,true,0,maxRes);                  // Widerstand (Ohm)
  voltage0 = inputNumber(ip4,1,true,5,20);                 // Maximalspannung (V)
...
function updateInput () {
  ip1.value = ToString(cap*1e6,0,true);                    // Kapazität (Mikrofarad)
  ip2.value = ToString(ind,2,true);                        // Induktivität (H)
  ip3.value = ToString(res,1,true);                        // Widerstand (Ohm)
  ip4.value = ToString(voltage0,1,true);                   // Maximalspannung (V)
  }
