...
var GAMMA = 6.67430e-11;                                   // Gravitationskonstante (SI)
...
var epsPlanets = [                                         // Numerische Exzentrizitäten
...
var ip1, ip2;                                              // Eingabefelder (große Halbachse, numerische Exzentrizität
...
var ePix;                                                  // Lineare Exzentrizität (Pixel)
...
function start () {
  ...
  getElement("ip2a",text03);                               // Erklärender Text (Numerische Exzentrizität)    
  ip2 = getElement("ip2b");                                // Eingabefeld (Numerische Exzentrizität)
  ...
  ip2.onkeydown = reactionEnter;                           // Reaktion auf Eingabe (Numerische Exzentrizität)
  ip1.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (große Halbachse)
  ip2.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (numerische Exzentrizität)
...
// Seiteneffekt phi1, phi2 b, a2, b2, bPix, ePix, c1, vMin, vMax, t, t0, phi1Min, phi1Max, phi2Min, phi2Max, t0S1, t0S2, tS1, tS2

function reaction () {
  input();                                                 // Eingabe
  phi1 = 0; phi2 = Math.PI;                                // St
  calculation();                                           // Berechnungen durchführen
  if (!on) paint();                                        // Neu zeichnen
  }
  
// Reaktion auf Auswahl in der Liste:
// Seiteneffekt t, unknown, a, eps, phi1, phi2, b, a2, b2, bPix, ePix, c1, vMin, vMax, t0, phi1Min, phi1Max, phi2Min, phi2Max, 
...
function reactionSelect () {
  ...
  if (i < n) {                                             // Falls normales Feld ausgewählt ...
    ...
    eps = epsPlanets[i];                                   // Numerische Exzentrizität
    updateInput();                                         // Eingabefelder aktualisieren
    }
  reaction();                                              // Eingabe, Berechnungen, Ausgabe, neu zeichnen
...
// Seiteneffekt a, eps, unknown, phi1, phi2, b, a2, b2, bPix, ePix, c1, vMin, vMax, t, t0, phi1Min, phi1Max, phi2Min, phi2Max,
...
// Seiteneffekt part, phi1, phi2 b, a2, b2, bPix, ePix, c1, vMin, vMax, t, t0, phi1Min, phi1Max, phi2Min, phi2Max, t0S1, t0S2, tS1, tS2
...
// Zeitunabhängige Berechnungen, Aktualisierung der beiden unteren Ausgabefelder:
// Seiteneffekt b, a2, b2, bPix, ePix, c1, vMin, vMax
...
function calculation () {
  ...
  ePix = eps*A_PIX;                                        // Lineare Exzentrizität (Pixel)
...
function input () {
  ...
  eps = inputNumber(ip2,3,true,0,0.999);                   // Numerische Exzentrizität
...
function updateInput () {
  ...
  ip2.value = ToString(eps,3,true);                        // Numerische Exzentrizität
  }
...
function planet () {
  ...
  var rRel = 1-eps*Math.cos(e);                            // Relativer Abstand zur Sonne (bezogen auf große Halbachse)
  ...
  var v = Math.sqrt(C0*(2/rAU-1/a));                       // Geschwindigkeit (m/s, Vis-Viva-Gleichung)
  op2.innerHTML = ToString(v/1000,3,false);                // Geschwindigkeit in km/s ausgeben
