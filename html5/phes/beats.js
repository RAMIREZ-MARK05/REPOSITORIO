...
var color1 = "#ff0000";                                    // Farbe für 1. Welle
var color2 = "#0000ff";                                    // Farbe für 2. Welle
var color3 = "#000000";                                    // Farbe für Gesamtwelle
...
// Funktion zur Berechnung der Elongation (Pixel) für 1. Welle:
// t0 ... Zeitpunkt entsprechend senkrechter Achse (ms)

function elongation1 (tS) {
  return AMPL*Math.sin(omega1*tS);                         // Rückgabewert
  }
  
// Funktion zur Berechnung der Elongation (Pixel) für 2. Welle:
// t0 ... Zeitpunkt entsprechend senkrechter Achse (ms)

function elongation2 (tS) {
  return AMPL*Math.sin(omega2*tS);                         // Rückgabewert
  }
  
// Funktion zur Berechnung der Elongation (Pixel) für Überlagerung:
// t0 ... Zeitpunkt entsprechend senkrechter Achse (ms)

function elongationSum (tS) {
  return AMPL*(Math.sin(omega1*tS)+Math.sin(omega2*tS));   // Rückgabewert
  }
  
// Funktionen zur Berechnung der Elongation (Pixel) für Einhüllende:
// t0 ... Zeitpunkt entsprechend senkrechter Achse (ms)

function elongationEnv1 (tS) {
  return 2*AMPL*(Math.cos(omegaMod*tS));                   // Rückgabewert
  }
...
function graph (v0, t0, e, c) {
  ...
  for (u=U0+1; u<=U0+400; u++) {                           // Für alle Werte der waagrechten Bildschirmkoordinate ...
...
function curves () {
  ...
  graph(V01,t0,elongation1,color1);                        // Graph für 1. Welle
  graph(V02,t0,elongation2,color2);                        // Graph für 2. Welle
  graph(V03,t0,elongationSum,color3);                      // Graph für Gesamtwelle
  graph(V03,t0,elongationEnv1,color4);                     // 1. Einhüllende
  graph(V03,t0,elongationEnv2,color4);                     // 2. Einhüllende
  }
...
function marks () {
  ...
  var e1 = elongation1(tS);                                // Elongation für 1. Welle (Pixel)
  var e2 = elongation2(tS);                                // Elongation für 2. Welle (Pixel)
  circle(u,V01-e1,2.5,color1);                             // Markierung für 1. Welle
  circle(u,V02-e2,2.5,color2);                             // Markierung für 2. Welle
  circle(u,V03-e1-e2,2.5,color3);                          // Markierung für Gesamtwelle
  }
