...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel newtonlaw2_de.js) abgespeichert.

// Farben:

var colorBackground = "#ffff00";                           // Hintergrundfarbe
var colorTrack = "#ffc040";                                // Farbe für Fahrbahn
var colorStop = "#ff0000";                                 // Farbe für Prellbock
var colorGlider = "#0000ff";                               // Farbe für Gleiter
var colorScale1 = "#000000";                               // Farbe 1 für Lineal
var colorScale2 = "#00ff00";                               // Farbe 2 für Lineal
var colorLB = "#000000";                                   // Farbe für Lichtschranke
var colorClock1 = "#808080";                               // Farbe für Uhr (Gehäuse)
var colorClock2 = "#000000";                               // Farbe für Uhr (Anzeige)
var colorClock3 = "#ff0000";                               // Farbe für Uhr (Ziffern)
...
function start () {
  ...
  bu1 = getElement("bu1",text01);                          // Schaltknopf (Ergebnisse löschen)   
  ...
  bu3.disabled = true;                                     // Schaltknopf zunächst deaktiviert
  getElement("ip1a",text04);                               // Erklärender Text (Masse des Gleiters)
  ...
  getElement("ip2a",text05);                               // Erklärender Text (Masse des Wägestücks)
  getElement("ip2b",symbolMass2+" = ");                    // Symbol (Masse des Wägestücks) und Gleichheitszeichen 
  ip2 = getElement("ip2c");                                // Eingabefeld (Masse des Wägestücks)
  getElement("ip2d",gram);                                 // Einheit (Masse des Wägestücks)
  getElement("ip3a",text06);                               // Erklärender Text (Reibungszahl)
  ...
  getElement("lbM",text07);                                // Erklärender Text (Messwerte)
  ...
  m2 = 0.001;                                              // Masse des Wägestücks (kg) 
  my = 0;                                                  // Reibungszahl
  ...
  bu1.onclick = reactionButton1;                           // Reaktion auf Schaltknopf 1 (Messreihe löschen)
  ...
  ip1.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Masse Gleiter)
  ip2.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (beschleunigende Masse)
  ip3.onblur = reaction;                                   // Reaktion auf Verlust des Fokus (Reibungszahl)  
...
function enableInput (p) {
  ip1.readOnly = !p;                                       // Eingabefeld für Masse des Gleiters
  ip2.readOnly = !p;                                       // Eingabefeld für beschleunigende Masse
  ip3.readOnly = !p;                                       // Eingabefeld für Reibungszahl
  }
  
// Reaktion auf Schaltknopf 1 (Messreihe löschen):
// Seiteneffekt on, timer, list, state, diagr, t, ls, m1, m2, my, a, xLB, tLB, t0, x
// Wirkung auf Schaltfläche
...
function reactionButton1 () {
  ...
  reaction();                                              // Eingegebene Werte, rechnen, neu zeichnen
...
function reactionButton2 () {
  reaction();                                              // Eingabe übernehmen und rechnen
  ...
  if (state >= 2) {                                        // Falls bisher Zustand nach Lichtschranke ...
    ...
    if (list.length > 2) bu3.disabled = false;             // Falls genügend viele Messungen, Diagramm-Schaltknopf aktivieren
    state = 0;                                             // Neuer Zustand: vor dem Start
    bu2.innerHTML = text02[0];                             // Text für Schaltknopf: Start 
...
function reactionButton3 () {
  diagr = true;                                            // Flag für Diagramm setzen
...
// Berechnungen für einzelne Messung:
...
function updateInput () {
  ...
  ip2.value = ToString(1000*m2,1,true);                    // Masse des Wägestücks (g)
...
function track () {
  ...
  newPath();                                               // Neuer Grafikpfad (Standardwerte)
  ctx.fillStyle = colorTrack;                              // Füllfarbe
  ...
  ctx.lineTo(10,60);                                       // Zurück zum Anfangspunkt
  ctx.fill(); ctx.stroke();                                // Fahrbahn ausfüllen und Rand zeichnen
...
function glider () {
  ...
  ctx.arc(xEnd+17,70,13,270*DEG,0,false);                  // Weiter mit Viertelkreis (gekrümmter Teil der Schnur)
  ctx.lineTo(xEnd+30,100+dx);                              // Weiter nach unten (senkrechter Teil der Schnur)
  ctx.stroke();                                            // Schnur zeichnen
  rectangle(xEnd+28,100+dx,4,6,colorGlider);               // Wägestück
...
function scale () {
  ...
  for (var i=1; i<10; i+=2)                                // Für jedes zweite Feld ... 
    rectangle(X0+i*20,85,20,10,colorScale2);               // Rechteck mit anderer Füllfarbe
  }
  
// Lichtschranke mit Doppelpfeil darüber:
    
function lightbarrier () {
  rectangle(ls-3,40,6,20,colorLB);                         // Lichtschranke
  line(ls-20,30,ls+20,30);                                 // Linie für Doppelpfeil über der Lichtschranke
...
function clock () {
  ...
  rectangle(u0-50,v0-15,100,30,colorClock1);               // Gehäuse
  rectangle(u0-40,v0-10,80,20,colorClock2);                // Anzeige
  ctx.fillStyle = colorClock3;                             // Farbe für Ziffern
  var s = value((t<tLB ? t : tLB),3,second);               // Zeichenkette für Zeit
  alignText(s,1,u0,v0+5,FONT2);                            // Zeichenkette anzeigen
  }
  
// Hilfsroutine für Zeit-Weg-Diagramm: Koordinatensystem
...
function axes (u0, v0) {
  ...
  alignText(symbolTime,1,u0+125,v0+15);                    // Symbol für Zeit
  alignText(text09,1,u0+125,v0+27);                        // Einheit (s)      
  for (var i=1; i<=5; i++) {                               // Für alle Ticks der Zeitachse ...
  ...
  alignText(symbolDisplacement,1,u0-20,v0-225);            // Symbol für Messstrecke
  alignText(text10,1,u0-20,v0-213);                        // Einheit (m) 
  for (i=1; i<=10; i++) {                                  // Für alle Ticks der Wegachse ...
...
// Hilfsroutine für Zeit-Weg-Diagramm: Messergebnis im Diagramm markieren
// (u0,v0) ... Ursprung (Pixel)
// x ......... Strecke (m)
...
// Hilfsroutine für Zeit-Weg-Diagramm: Parabel
// (u0,v0) ... Ursprung (Pixel)

function parabola (u0, v0) {
  var u = u0, v = v0;                                      // Koordinaten des Ursprungs übernehmen (Pixel)
  newPath();                                               // Neuer Grafikpfad (Standardwerte)
  ...
  for (u = u0; u<u0+140; u++) {                            // Von links nach rechts ...
    ...
    if (v < v0-220) {v = v0-220; break;}                   // Falls oberer Rand überschritten, abbrechen
    ctx.lineTo(u,v);                                       // Linie zum Grafikpfad hinzufügen
    }
  ctx.stroke();                                            // Polygonzug als Näherung für Parabel zeichnen
  }
...
function diagram () {
  ...
  for (var i=0; i<list.length; i++)                        // Für alle bisherigen Messergebnisse ...
    dataPoint(u0,v0,list[i]);                              // Markierung         
  if (state >= 2)                                          // Falls Lichtschranke schon passiert ...
    dataPoint(u0,v0,xLB);                                  // Markierung für neues Messergebnis
  if (diagr) parabola(u0,v0);                              // Falls gewünscht, Parabel zeichnen    
  var tt = Math.min(t,Math.sqrt(2/a));                     // Zeit (s) für aktuelles Messergebnis
  var u = u0+tt*PIX_T;                                     // Waagrechte Koordinate (Pixel)                                             
  var v = v0-(a/2)*tt*tt*LENGTH;                           // Senkrechte Koordinate (Pixel)
  circle(u,v,2.5,"#ff0000");                               // Markierung für aktuelles Messergebnis
  }
...
function writeValues () {
  ...
  ctx.fillStyle = "#000000";                               // Schriftfarbe
  var str = symbolDisplacement+" = "+value(xLB,3,meter);   // Zeichenkette für Strecke s
  alignText(str,0,u0,v0);                                  // Zeichenkette ausgeben
  str = symbolTime+" = ";                                  // Anfang der Zeichenkette für Zeit t
  if (state >= 2) str += value(tLB,3,second);              // Falls Messung durchgeführt, Zeichenkette ergänzen
  alignText(str,0,u0,v0+20);                               // Zeichenkette ausgeben
  str = symbolAcceleration+" =  ";                         // Anfang der Zeichenkette für Beschleunigung a
  alignText(str,0,u0,v0+45);                               // Bisherige Zeichenkette ausgeben 
  var w0 = ctx.measureText(str).width;                     // Länge der bisherigen Zeichenkette
  line(u0+w0,v0+40,u0+20+w0,v0+40,"#000000",2);            // Bruchstrich
  alignText("2"+symbolDisplacement,1,u0+10+w0,v0+37);      // Zähler 2s
  alignText(symbolTime+"\u00B2",1,u0+10+w0,v0+53);         // Nenner t²
  if (state <= 1) return;                                  // Falls Messung noch nicht durchgeführt, abbrechen 
  str = " = "+value(a,3,meterPerSecond2);                  // Zeichenkette für Wert der Beschleunigung
...
function paint () {
  ...
  if (state == 1 && x > xLB) {                             // Falls Lichtschranke inzwischen passiert ...
    ...
    bu2.innerHTML = text02[1];                             // Text des zweiten Schaltknopfs ändern (Registrieren)
