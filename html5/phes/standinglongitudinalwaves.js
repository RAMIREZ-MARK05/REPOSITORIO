...
var C_SI = 343.5;                                          // Schallgeschwindigkeit real (20 °C, m/s)
var C_PIX = 2000;                                          // Schallgeschwindigkeit der Animation (Pixel/s)
var LEN = 300;                                             // Rohrlänge (Pixel)
...
function start () {
  ...
  focus(ipL);                                              // Fokus für Eingabefeld
...
// Hilfsroutine: Eingabe übernehmen, rechnen, Ausgabefelder aktualisieren, neu zeichnen
...
function reactionDown (x, y) {
  ...
  if (x >= LEFT && x <= LEFT+LEN && y >= 30 && y <= 90)    // Falls Position im Bereich des Rohrs ... 
...
function calculation () {
  ...
  lambda = factor*LEN;                                     // Wellenlänge (Pixel)
  lambda4 = lambda/4;                                      // Viertel der Wellenlänge (Pixel)
  omega = C_PIX/(lambda*2*Math.PI);                        // Kreisfrequenz der Animation (1/s) 
  k = 2*Math.PI/lambda;                                    // Wellenzahl (1/Pixel)
  nyReal = C_SI/lambdaReal;                                // Reale Frequenz (Hz)
...
function drawMarks (n, y) {
  ...
  var iMax = Math.round(LEN/lambda4);                      // Maximaler Index für Markierungen
...
function drawTube () {
  ...
  ctx.fillRect(LEFT,27,LEN,3);                             // Oberer Rand
  ctx.fillRect(LEFT,90,LEN,3);                             // Unterer Rand 
  if (type >= 1) ctx.fillRect(LEFT-3,27,3,66);             // Linker Rand
  if (type == 2) ctx.fillRect(LEFT+LEN,27,3,66);           // Rechter Rand
  drawMarks(0,27);                                         // Markierungen oben 
  drawMarks(0,92);                                         // Markierungen unten
  for (var x=LEFT+10; x<=LEFT+LEN-10; x+=20)               // Für die erste, dritte, fünfte Reihe usw. ...              
    seriesParticles(x,40,80);                              // Teilchen zeichnen
  for (x=LEFT; x<=LEFT+LEN; x+=20)                         // Für die zweite, vierte, sechste Reihe usw. ...
...
function drawDiagram (n, x0, y0) {
  var right = LEFT+LEN;                                    // Rechter Rand (Pixel)
