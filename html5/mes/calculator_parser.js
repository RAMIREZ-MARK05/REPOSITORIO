...
function partsProd0 (s) {
  for (var i=s.length-1; i>0; i--) {                       // F�r alle Zeichenpositionen (von rechts nach links)
    ...
    if (isNum(s1) && isBrack(s2)                           // Falls Zahl mal Klammer ...
       || isCompleteBrack(s1) && isBrack(s2))              // ... oder vollst�ndige Klammer mal Klammer ...
      return [s1, s2];                                     // R�ckgabewert (1. und 2. Faktor)
    }
  return undefined;                                        // R�ckgabewert, falls keine passende Zerlegung gefunden
  }
