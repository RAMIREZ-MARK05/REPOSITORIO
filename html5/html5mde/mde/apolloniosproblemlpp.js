// Apollonios-Problem LPP (eine Gerade, zwei Punkte)
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel apolloniosproblemlpp_de.js) abgespeichert.
...
var nr;                                                    // Nummer des ausgewählten Objekts (0 bis 4)
var g;                                                     // Gegebene Gerade
var p1, p2;                                                // Gegebene Punkte (Kreise mit Radius 0)
...
function initLine (i) {
  ...
  initCheckbox(n+"1",i==1);                                // Passives Optionsfeld (Halbebene)
...
function start () {
  ...
  cb = new Array(2);                                       // Array von Optionsfeldern (Lösungstypen)
  lb = new Array(2);                                       // Array von Ausgabefeldern (Anzahl Lösungskreise)
  for (var i=0; i<2; i++) initLine(i);                     // Zeilen für Lösungstypen vorbereiten
  getElement("text4",text04);                              // Erklärender Text (Zahl der Lösungen)
  g = {x1: 50, y1: 350, x2: 280, y2: 320};                 // Gegebene Gerade
  p1 = {x: 200, y: 100, r: 0};                             // Erster gegebener Punkt (Kreis mit Radius 0)
  p2 = {x: 120, y: 240, r: 0};                             // Zweiter gegebener Punkt (Kreis mit Radius 0)  
...
function reactionDown (u, v) {
  ...
  var d = distancePoint(u,v,p1.x,p1.y);                    // Abstand vom ersten Punkt
  var dMin = d;                                            // Vorläufiges Abstandsminimum
  var n = 1;                                               // Nummer für ersten Punkt
  d = distancePoint(u,v,p2.x,p2.y);                        // Abstand vom zweiten Punkt
...
function reactionMove (u, v) {
  ...
  if (nr == 1) {p1.x = u; p1.y = v;}                       // Ersten gegebenen Punkt anpassen
  if (nr == 2) {p2.x = u; p2.y = v;}                       // Zweiten gegebenen Punkt anpassen
  if (nr == 3) {g.x1 = u; g.y1 = v;}                       // Punkt 1 der Gerade anpassen
  if (nr == 4) {g.x2 = u; g.y2 = v;}                       // Punkt 2 der Gerade anpassen
  paint();                                                 // Neu zeichnen
  }
...
function drawSolutions (i) {
  ...
  var a = solution(s3);                                    // Array der Lösungskreise
...
function paint () {
  ...
  for (var i=0; i<2; i++) sum += drawSolutions(i);         // Lösungskreise der einzelnen Lösungstypen
