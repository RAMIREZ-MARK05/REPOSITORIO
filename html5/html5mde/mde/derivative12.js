...
// Java-Applet (10.10.1999) umgewandelt
// 12.11.2015 - 09.08.2023
...
// Sprachabhängige Texte sind in einer eigenen Datei (zum Beispiel derivative12_de.js) abgespeichert.
...
var colorDerivative2 = "#0000ff";                          // Farbe für 2. Ableitung
...
var upn2;                                                  // UPN-Array für 2. Ableitungsfunktion
...
function start () {
  ...
  cb1 = getElement("cb1");                                 // Optionsfeld (1. Ableitung)
  cb1.checked = true;                                      // Häkchen gesetzt
  getElement("cb1a",text03);                               // Erklärender Text (1. Ableitung)
  cb2 = getElement("cb2");                                 // Optionsfeld (2. Ableitung)
  cb2.checked = false;                                     // Häkchen nicht gesetzt
  getElement("cb2a",text04);                               // Erklärender Text (2. Ableitung)
  getElement("ip2a",text05);                               // Erklärender Text (linker Rand)
  ip2 = getElement("ip2b");                                // Eingabefeld (linker Rand)
  getElement("ip3a",text06);                               // Erklärender Text (rechter Rand)
  ip3 = getElement("ip3b");                                // Eingabefeld (rechter Rand)
  getElement("ip4a",text07);                               // Erklärender Text (unterer Rand)
  ip4 = getElement("ip4b");                                // Eingabefeld (unterer Rand)
  getElement("op5a",text08);                               // Erklärender Text (oberer Rand)
  op5 = getElement("op5b");                                // Ausgabefeld (oberer Rand)
  bu = getElement("bu",text09);                            // Schaltknopf
  ...
  ip1.onblur = reactionButton;                             // Reaktion auf Verlust des Fokus (Eingabefeld Funktionsterm)
  ip2.onblur = reactionButton;                             // Reaktion auf Verlust des Fokus (Eingabefeld linker Rand)
  ip3.onblur = reactionEnter;                              // Reaktion auf Verlust des Fokus (Eingabefeld rechter Rand)
  ip4.onblur = reactionEnter;                              // Reaktion auf Verlust des Fokus (Eingabefeld unterer Rand)
  cb1.onchange = reactionButton;                           // Reaktion auf Optionsfeld (1. Ableitung)
  cb2.onchange = reactionButton;                           // Reaktion auf Optionsfeld (2. Ableitung)
...
// Seiteneffekt xMin, xMax, yMin, yMax, pix, upn, upn1, upn2, Wirkung auf Ein- und Ausgabefelder 
...
function reactionButton () {
  ...
  if (!isCorrect(upn)) {                                   // Falls Funktionsterm unverständlich ...
    alert(text10);                                         // Warnmeldung                                        
    return;                                                // Abbrechen
    }                      
  term = f;                                                // Neuen Funktionsterm speichern
  ip1.value = f.replace(".",decimalSeparator);
  upn1 = derivative(upn);                                  // UPN-Array der 1. Ableitungsfunktion
  upn2 = derivative(upn1);                                 // UPN-Array der 2. Ableitungsfunktion
  paint();                                                 // Neu zeichnen
...
function paint () {
  ...
  ctx.strokeStyle = colorDerivative1;                      // Farbe für 1. Ableitung
  if (cb1.checked) plotGraph(upn1);                        // Falls gewünscht, Graph der 1. Ableitungsfunktion
  ctx.strokeStyle = colorDerivative2;                      // Farbe für 2. Ableitungsfunktion
  if (cb2.checked) plotGraph(upn2);                        // Falls gewünscht, Graph der 2. Ableitungsfunktion
  }
