// Krümmungskreise von Funktionsgraphen
// 06.09.2017 - 08.08.2023
...
// Sprachabhängige Texte sind in einer eigenen Datei (zum Beispiel osculatingcircle_de.js) abgespeichert.
...
function start () {
  ...
  ip1.onblur = reactionButton;                             // Reaktion auf Verlassen des Eingabefelds Funktionsterm
  ip2.onblur = reactionButton;                             // Reaktion auf Verlassen des Eingabefelds Linker Rand
  ip3.onblur = reactionButton;                             // Reaktion auf Verlassen des Eingabefelds Rechter Rand
  ip4.onblur = reactionButton;                             // Reaktion auf Verlassen des Eingabefelds Unterer Rand
...
function reactionButton () {
  ...
  if (ae == ip3) focus(ip4);                               // Fokus für das nächste Eingabefeld
...
// Zeichnen der Tangente in (x0,y0):
...
// Zeichnen des Berührkreises:
// x0, y0 ... Koordinaten des Berührpunkts

function drawCircle (x0, y0) {
  var m = value(upn1,x0);                                  // Steigung
  var u0 = (x0-xMin)*pix, v0 = (yMax-y0)*pix;              // Bildschirmkoordinaten des Berührpunkts
  var y1 = value(upn1,x0);                                 // 1. Ableitung: f'(x0)
  var y2 = value(upn2,x0);                                 // 2. Ableitung: f''(x0)
  if (y2 == 0) return;                                     // Falls 2. Ableitung gleich 0, abbrechen
  var h1 = 1+y1*y1;                                        // Hilfsgröße: 1+(f'(x0))^2
  var h2 = h1/y2;                                          // Hilfsgröße: (1+(f'(x0))^2)/f''(x0)
  var xM = x0-y1*h2, yM = y0+h2;                           // Koordinaten des Kreismittelpunkts
  var uM = (xM-xMin)*pix, vM = (yMax-yM)*pix;              // Bildschirmkoordinaten des Kreismittelpunkts
  ctx.strokeStyle = "#000000";                             // Linienfarbe
  line(u0,v0,(xM-xMin)*pix,(yMax-yM)*pix);                 // Verbindungslinie (Kreisradius)
  drawPoint(xM,yM,colorDerivative2);                       // Kreismittelpunkt zeichnen
  var r = Math.abs(Math.pow(h1,1.5)/y2);                   // Kreisradius
  newPath();                                               // Neuer Grafikpfad (Standardwerte)
  ctx.arc(uM,vM,r*pix,0,2*Math.PI,true);                   // Kreis vorbereiten
  ctx.strokeStyle = colorDerivative2;                      // Linienfarbe
...
function paint () {
  ...
  drawTangent(x0,y0);
  drawCircle(x0,y0);                                       // Krümmungskreis
