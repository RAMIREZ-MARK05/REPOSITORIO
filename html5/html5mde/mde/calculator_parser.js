...
function partsProd0 (s) {
  for (var i=s.length-1; i>0; i--) {                       // Für alle Zeichenpositionen (von rechts nach links)
    var s1 = s.substring(0,i);                             // 1. Teil der Zeichenkette
    var s2 = s.substring(i);                               // 2. Teil der Zeichenkette
    if (isNum(s1) && isBrack(s2)                           // Falls Zahl mal Klammer ...
       || isCompleteBrack(s1) && isBrack(s2))              // ... oder vollständige Klammer mal Klammer ...
      return [s1, s2];                                     // Rückgabewert (1. und 2. Faktor)
    }
  return undefined;                                        // Rückgabewert, falls keine passende Zerlegung gefunden
  }
