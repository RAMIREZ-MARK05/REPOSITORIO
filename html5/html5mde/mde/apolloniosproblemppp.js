...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel apolloniosproblemppp_de.js) abgespeichert.
...
var lb;                                                    // Ausgabefeld (Zahl der Lösungen)
var nr;                                                    // Nummer des ausgewählten Punkts (0 bis 3)
...
function start () {
  ...
  getElement("text2a",text02[0]);                          // Erklärender Text (Aufgabenstellung)
  getElement("text2b",text02[1]);                          // Erklärender Text (Aufgabenstellung)
  getElement("text2c",text02[2]);                          // Erklärender Text (Aufgabenstellung)
  getElement("text2d",text02[3]);                          // Erklärender Text (Aufgabenstellung)
  lb = getElement("lbn");                                  // Ausgabefeld (Anzahl Lösungskreise)
...
function solution () {
  ...
  var h = p1.r+c1*e1+c2*e2;                                // Summand außerhalb der Wurzel
...
function drawSolutions () {
  var a = solution();                                      // Array der Lösungskreise
  if (a[0]) drawCircle(a[0],false,color2);                 // Gegebenenfalls 1. Lösungskreis zeichnen
  if (a[1]) drawCircle(a[1],false,color2);                 // Gegebenenfalls 2. Lösungskreis zeichnen
  var n = numberCircles(a);                                // Zahl der Lösungen
  lb.innerHTML = (n==1 ? ""+n : "&infin;");                // Lösungszahl in der Schaltfläche aktualisieren
  return n;                                                // Rückgabewert
  }
