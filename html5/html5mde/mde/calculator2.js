// Online-Rechner Algebra
// 25.04.2020 - 17.08.2020
...
var variables0 = "abcdefghijklmnopqrstuvwxyz";             // Erlaubte Variablennamen
...
var variables;                                             // Zeichenkette der vorkommenden Variablen
...
function start () {
  ...
  getElement("var",text12);                                // Erklärender Text (Variable)
  selVar = newSelect("selVar",text13);                     // Auswahlfeld für Variable
...
function reactionReset () {
  variables = "";                                          // Zeichenkette der vorkommenden Variablen
...
function reactionVar () {
  var i = selVar.selectedIndex;                            // Ausgewählter Index
  var ch = variables0.charAt(i-1);                         // Ausgewählter Buchstabe
  if (variables.indexOf(ch) < 0) variables += ch;          // Falls neue Variable, Zeichenkette der Variablen erweitern
  append(ch);                                              // Variable hinzufügen, Termkonstruktion, Grafikausgabe
...
function paint2 (y) {
  ...
  ctx.fillText(text21,x1,y);                               // Erklärender Text (Zeichenkette)
  ctx.fillText(input,x2,y);                                // Zeichenkette
  ctx.fillText("complete = "+te.complete,x1,y+20);         // Vollständigkeit
  ctx.fillText("x = "+te.x,x1,y+40);                       // x-Koordinate (Pixel)
  ctx.fillText("y = "+te.y,x2,y+40);                       // y-Koordinate (Pixel)
  ctx.fillText("width = "+te.width,x1,y+60);               // Breite (Pixel)
  ctx.fillText("asc = "+te.asc,x2,y+60);                   // Platzbedarf nach oben (Pixel)
  ctx.fillText("desc = "+te.desc,x3,y+60);                 // Platzbedarf nach unten (Pixel)
  ctx.fillText("variables: "+variables,x1,y+80);           // Vorkommende Variablen
  }
  
// Für Tests: Ausgabe eines Arrays (Länge und einzelne Elemente)
  
function alertA (a) {
  alert("L = "+a.length);
  for (var i=0; i<a.length; i++) alert("i = "+i+":  "+a[i]);
  }
...
function paint () {
  ...
  ctx.fillText(text24,x1,400);                             // Erklärender Text (Kommentar)
  ctx.fillText(text25,x1,200);                             // Erklärender Text (Ergebnis)
  ctx.fillText(text26,x1,300);                             // Erklärender Text (Faktorisierung)
  ctx.font = FONT2;                                        // Zeichensatz 2 (Monospace)
  ctx.fillStyle = "#000000";                               // Schriftfarbe
  te.write(x2,100);                                        // Term ausgeben
  ctx.fillText(te.comment(),x2,400);                       // Bemerkung ausgeben
  if (te.value != undefined) {                             // Falls Wert des Terms definiert ...
    te.value.write(x2,200);                                // Ausmultipliziertes Ergebnis ausgeben
    te.value.writeFactors(x2,300);                         // Faktorisierung ausgeben
    }
  else {                                                   // Falls Wert des Terms undefiniert ...
    ctx.fillText("?",x2,200);                              // Fragezeichen für ausmultipliziertes Ergebnis
    ctx.fillText("?",x2,300);                              // Fragezeichen für Faktorisierung
    }
  //paint2(400);                                             // Angaben für Fehlersuche
  }
