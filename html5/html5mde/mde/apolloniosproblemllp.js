// Apollonios-Problem LLP (zwei Geraden, ein Punkt)
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel apolloniosproblemllp_de.js) abgespeichert.
...
function initLine (i) {
  ...
  initCheckbox(n+"1",i>=2);                                // Passives Optionsfeld (Halbebene bezüglich 1. Gerade)
  initCheckbox(n+"2",i%2==1);                              // Passives Optionsfeld (Halbebene bezüglich 2. Gerade)
...
function start () {
  ...
  g1 = {x1: 20, y1: 300, x2: 400, y2: 160};                // 1. gegebene Gerade 
  g2 = {x1: 20, y1: 120, x2: 400, y2: 250};                // 2. gegebene Gerade 
  p = {x: 200, y: 120, r: 0};                              // Gegebener Punkt (Kreis mit Radius 0)
...
function reactionDown (u, v) {
  ...
  var d = distancePoint(u,v,p.x,p.y);                      // Abstand vom gegebenen Punkt
  var dMin = d;                                            // Vorläufiges Abstandsminimum
  var n = 1;                                               // Nummer für Punkt
  d = distanceLine(u,v,g1);                                // Abstand von Gerade 1
  if (d < dMin) {                                          // Falls Abstand kleiner als bisheriges Minimum ...
    ...
    n = (d1<d2 ? 2 : 3);                                   // Nummer aktualisieren
    }
...
function reactionMove (u, v) {
  ...
  if (nr == 1) {p.x = u; p.y = v;}                         // Punkt anpassen
  if (nr == 2) {g1.x1 = u; g1.y1 = v;}                     // Ersten Punkt von Gerade 1 anpassen
  if (nr == 3) {g1.x2 = u; g1.y2 = v;}                     // Zweiten Punkt von Gerade 1 anpassen
  if (nr == 4) {g2.x1 = u; g2.y1 = v;}                     // Ersten Punkt von Gerade 2 anpassen
  if (nr == 5) {g2.x2 = u; g2.y2 = v;}                     // Zweiten Punkt von Gerade 2 anpassen
  paint();                                                 // Neu zeichnen
  }
...
// s2, s3 ... Vorzeichenfaktoren bezüglich Geraden (Halbebenen)
// Rückgabewert: Array mit 2 Elementen (Lösungskreis bzw. undefined)

function solution (s2, s3) {
  ...
  lin[0] = coeffLinEquPar(g1,s2);                          // Koeffizienten der ersten linearen Gleichung
  lin[1] = coeffLinEquPar(g2,s3);                          // Koeffizienten der zweiten linearen Gleichung
  ...
  // Einsetzen in Berühr-Bedingung für Kreis
  var e1 = d1-p.x;                                         // Konstanter Summand
  var e2 = d2-p.y;                                         // Konstanter Summand
  // Quadratische Gleichung für r: (c1*r+e1)^2 + (c2*r+e2)^2 = (r-s1*r1)^2
  // Gelöst mit Maxima!
  var denom = 1-c1*c1-c2*c2;                               // Nenner
  var discr = (1-c1*c1)*e2*e2 + (1-c2*c2)*e1*e1 + 2*c1*c2*e1*e2;     // Diskriminante
  if (discr < 0) return [undefined, undefined];            // Rückgabewert für negative Diskriminante
  var root = Math.sqrt(discr);                             // Wurzel
  var h = c1*e1+c2*e2;                                     // Summand außerhalb der Wurzel
...
function circle2x (xM, yM, r, sgn, c) {
  ...
  newPath(c);                                              // Neuer Grafikpfad für Viertelkreis oder Teil davon
...
function drawSolutions (i) {
  var s2 = (i<2 ? +1 : -1);                                // Vorzeichenfaktor 1. Gerade
  var s3 = (i%2==0 ? +1 : -1);                             // Vorzeichenfaktor 2. Gerade 
  var a = solution(s2,s3);                                 // Array der Lösungskreise
