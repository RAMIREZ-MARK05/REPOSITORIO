// Apollonios-Problem CPP (ein Kreis, zwei Punkte)
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel apolloniosproblemcpp_de.js) abgespeichert.
...
function initLine (i) {
  ...
  initCheckbox(n+"1",i%2==1);                              // Passives Optionsfeld (ein-/ausschließend)
...
function start () {
  ...
  p1 = {x: 120, y: 240, r: 0};                             // 1. gegebener Punkt (Kreis mit Radius 0)
  p2 = {x: 260, y: 300, r: 0};                             // 2. gegebener Punkt (Kreis mit Radius 0)
...
function reactionDown (u, v) {
  ...
  var d = distance(u,v,k);                                 // Abstand vom Kreismittelpunkt
  ...
  d = distance(u,v,p1);                                    // Abstand vom Punkt 1
...
function reactionMove (u, v) {
  ...
  if (nr == 2) k.r = distance(u,v,k);                      // Kreisradius anpassen
  if (nr == 3) {p1.x = u; p1.y = v;}                       // Punkt 1 anpassen
  if (nr == 4) {p2.x = u; p2.y = v;}                       // Punkt 2 anpassen
  paint();                                                 // Neu zeichnen
  }
...
// s1 ... Vorzeichenfaktor (+1 für einschließende, -1 für ausschließende Berührung)
...
function solution (s1) {
  ...
  lin[0] = coeffLinEqu(k,s1,p1,1);                         // Koeffizienten der ersten linearen Gleichung
  lin[1] = coeffLinEqu(p1,1,p2,1);                         // Koeffizienten der zweiten linearen Gleichung
...
function drawPoint (p, c, n) {
  ...
  ctx.fillStyle = c; ctx.fill();                           // Ausgefüllten Kreis zeichnen
...
function drawSolutions (i) {
  var s1 = (i%2==0 ? +1 : -1);                             // Vorzeichenfaktor
  var a = solution(s1);                                    // Array der Lösungskreise
