// Dreiecks-Labor: Exeter-Punkt
// Java-Applet (24.11.2004) umgewandelt
// 01.02.2017 - 27.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_exeterpoint_de.js) abgespeichert.
...
var color3 = "#40ff40";                                    // Farbe für Tangentendreieck
...
var S1, S2, S3;                                            // Schnittpunkte der verlängerten Seitenhalbierenden mit dem Umkreis
var E;                                                     // Exeter-Punkt
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, U, T1, T2, T3, S1, S2, S3, E
...
function update () {
  ...
  M1 = pointBarycentric(0,1,1);
  M2 = pointBarycentric(1,0,1);
  M3 = pointBarycentric(1,1,0);
  ...
  S1 = pointBarycentric(-a*a,b*b+c*c,b*b+c*c);             // Schnittpunkt von s_a mit dem Umkreis
  S2 = pointBarycentric(c*c+a*a,-b*b,c*c+a*a);             // Schnittpunkt von s_b mit dem Umkreis
  S3 = pointBarycentric(a*a+b*b,a*a+b*b,-c*c);             // Schnittpunkt von s_c mit dem Umkreis     
  var a2 = a*a, b2 = b*b, c2 = c*c;                        // Quadrate der Seitenlängen
  var a4 = a2*a2, b4 = b2*b2, c4 = c2*c2;                  // 4. Potenzen der Seitenlängen
  var e1 = a2*(b4+c4-a4);                                  // 1. baryzentrische Koordinate des Exeter-Punkts
  var e2 = b2*(c4+a4-b4);                                  // 2. baryzentrische Koordinate des Exeter-Punkts
  var e3 = c2*(a4+b4-c4);                                  // 3. baryzentrische Koordinate des Exeter-Punkts
  E = pointBarycentric(e1,e2,e3);                          // Exeter-Punkt 
  }
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, U, T1, T2, T3, S1, S2, S3, E
...
function reactionNext () {
  ...
  if (step >= 13) bu2.disabled = true;                     // Falls nötig, Schaltknopf "Nächster Schritt" deaktivieren
...
function paint () {
  ...
  triangle(T1,T2,T3,color3);                               // Ausgefülltes Tangentendreieck
  ...
  drawLinePP(T2,T3,color1);                                // Umkreistangente in A
  drawLinePP(T3,T1,color1);                                // Umkreistangente in B
  drawLinePP(T1,T2,color1);                                // Umkreistangente in C
  if (step >= 1) drawRayPP(A,M1,color1);                   // Verlängerte Seitenhalbierende s_a
  if (step >= 3) drawRayPP(B,M2,color1);                   // Verlängerte Seitenhalbierende s_b
  if (step >= 5) drawRayPP(C,M3,color1);                   // Verlängerte Seitenhalbierende s_c
  if (step >= 8) drawRayPP(T1,S1,color1);                  // 1. Verbindungslinie für Exeter-Punkt
  if (step >= 10) drawRayPP(T2,S2,color1);                 // 2. Verbindungslinie für Exeter-Punkt
  if (step >= 12) drawRayPP(T3,S3,color1);                 // 3. Verbindungslinie für Exeter-Punkt
  drawVertex(A,vertex1);                                   // Ecke A
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step >= 2) drawPoint(S1,color1);                     // Schnittpunkt von s_a mit dem Umkreis
  if (step >= 4) drawPoint(S2,color1);                     // Schnittpunkt von s_b mit dem Umkreis
  if (step >= 6) drawPoint(S3,color1);                     // Schnittpunkt von s_c mit dem Umkreis
  if (step >= 7) drawPoint(T1,color1);                     // 1. Ecke Tangentendreieck   
  if (step >= 9) drawPoint(T2,color1);                     // 2. Ecke Tangentendreieck
  if (step >= 11) drawPoint(T3,color1);                    // 3. Ecke Tangentendreieck
  if (step >= 13) drawPoint(E,color2,exeterpoint);         // Exeter-Punkt
  }
