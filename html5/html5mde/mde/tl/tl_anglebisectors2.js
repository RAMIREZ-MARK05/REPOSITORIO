// Dreiecks-Labor: Winkelhalbierende (Innen- und Außenwinkel)
// Java-Applet (28.10.2004) umgewandelt
// 10.02.2017 - 25.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_anglebisectors2_de.js) abgespeichert.
...
var color3 = "#40ffff";                                    // Farbe für Innenwinkel
var color4 = "#40ff40";                                    // Farbe für Außenwinkel
...
var A1, A2, A3;                                            // Mittelpunkte der Ankreise
var T12, T13, T21, T23, T31, T32;                          // Berührpunkte der Außenwinkelhalbierenden
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, I, A1, A2, A3, T12, T13, T21, T23, T31, T32
...
function update () {
  ...
  T12 = pointBarycentric(-tan1,0,1/tan3);                  // Berührpunkt Ankreis 1 - Verlängerung b
  T13 = pointBarycentric(-tan1,1/tan2,0);                  // Berührpunkt Ankreis 1 - Verlängerung c
  T21 = pointBarycentric(0,-tan2,1/tan3);                  // Berührpunkt Ankreis 2 - Verlängerung a
  T23 = pointBarycentric(1/tan1,-tan2,0);                  // Berührpunkt Ankreis 2 - Verlängerung c
  T31 = pointBarycentric(0,1/tan2,-tan3);                  // Berührpunkt Ankreis 3 - Verlängerung a
  T32 = pointBarycentric(1/tan1,0,-tan3);                  // Berührpunkt Ankreis 3 - Verlängerung b
  }
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, I, A1, A2, A3, T12, T13, T21, T23, T31, T32
...
function paint () {
  ...
  if (step >= 1) drawAngle(B,A,C,color3);                  // Winkel alpha
  if (step >= 2) drawAngle(C,B,A,color3);                  // Winkel beta
  if (step >= 3) drawAngle(A,C,B,color3);                  // Winkel gamma
  if (step >= 4) drawAngle(T32,A,B,color4);                // Außenwinkel zu alpha
  if (step >= 4) drawAngle(C,A,T23,color4);                // Außenwinkel zu alpha
  if (step >= 5) drawAngle(A,B,T31,color4);                // Außenwinkel zu beta
  if (step >= 5) drawAngle(T13,B,C,color4);                // Außenwinkel zu beta
  if (step >= 6) drawAngle(B,C,T12,color4);                // Außenwinkel zu gamma
  if (step >= 6) drawAngle(T21,C,A,color4);                // Außenwinkel zu gamma  
  drawLinePP(B,C);                                         // Seite a (verlängert)
  drawLinePP(C,A);                                         // Seite b (verlängert)
  drawLinePP(A,B);                                         // Seite c (verlängert)
  if (step >= 1) drawRayPP(A,I,color1);                    // Winkelhalbierende von alpha (Halbgerade)
  if (step >= 2) drawRayPP(B,I,color1);                    // Winkelhalbierende von beta (Halbgerade)
  if (step >= 3) drawRayPP(C,I,color1);                    // Winkelhalbierende von gamma (Halbgerade)
  if (step >= 4) drawLinePP(A,A2,color1);                  // Winkelhalbierende der Außenwinkel zu alpha (Gerade)
  if (step >= 5) drawLinePP(B,A3,color1);                  // Winkelhalbierende der Außenwinkel zu beta (Gerade)
  if (step >= 6) drawLinePP(C,A1,color1);                  // Winkelhalbierende der Außenwinkel zu gamma (Gerade)
  drawVertex(A,vertex1);                                   // Ecke A
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step >= 7) drawPoint(I,color2);                      // Inkreismittelpunkt
  if (step >= 8) drawPoint(A1,color2);                     // 1. Ankreismittelpunkt
  if (step >= 9) drawPoint(A2,color2);                     // 2. Ankreismittelpunkt
  if (step >= 10) drawPoint(A3,color2);                    // 3. Ankreismittelpunkt
  }
