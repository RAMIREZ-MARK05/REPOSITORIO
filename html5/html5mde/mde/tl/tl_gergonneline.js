// Dreiecks-Labor: Gergonne-Gerade
// 06.03.2017 - 27.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_gergonneline_de.js) abgespeichert.
...
var G1, G2, G3;                                            // Schnittpunkte auf Gergonne-Gerade
...
function start () {
  ...
  begin(170,230,230,230,160,140);                          // Anfangszustand
  paint();                                                 // Zeichnen
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, I, T1, T2, T3, rIC, G1, G2, G3
...
function update () {
  ...
  var t1 = 1/(s-a), t2 = 1/(s-b), t3 = 1/(s-c);            // Baryzentrische Koordinaten  
  T1 = pointBarycentric(0,t2,t3);                          // Berührpunkt auf [BC]
  T2 = pointBarycentric(t1,0,t3);                          // Berührpunkt auf [CA]
  T3 = pointBarycentric(t1,t2,0);                          // Berührpunkt auf [AB]  
  rIC = Math.sqrt((s-a)*(s-b)*(s-c)/s);                    // Inkreisradius
  G1 = pointBarycentric(0,t2,-t3);                         // 1. Schnittpunkt auf der Gergonne-Gerade
  G2 = pointBarycentric(-t1,0,t3);                         // 2. Schnittpunkt auf der Gergonne-Gerade
  G3 = pointBarycentric(t1,-t2,0);                         // 3. Schnittpunkt auf der Gergonne-Gerade
  }
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, I, T1, T2, T3, rIC, G1, G2, G3
...
function paint () {
  ...
  if (step >= 4) drawLinePP(T2,T3,color1);                 // Verbindungsgerade der Berührpunkte bei A
  if (step >= 5) drawLinePP(T3,T1,color1);                 // Verbindungsgerade der Berührpunkte bei B
  if (step >= 6) drawLinePP(T1,T2,color1);                 // Verbindungsgerade der Berührpunkte bei C
  if (step >= 7) drawLinePP(G1,G2,color2);                 // Gergonne-Gerade
  ...
  if (step >= 4) drawPoint(G1,color1);                     // 1. Schnittpunkt auf der Gergonne-Gerade
  if (step >= 5) drawPoint(G2,color1);                     // 2. Schnittpunkt auf der Gergonne-Gerade
  if (step >= 6) drawPoint(G3,color1);                     // 3. Schnittpunkt auf der Gergonne-Gerade
  }
