// Dreiecks-Labor: Gergonne-Punkt
// Java-Applet (28.10.2004) umgewandelt
// 14.01.2017 - 22.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_gergonnepoint_de.js) abgespeichert.
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, I, T1, T2, T3, G, rIC
...
function update () {
  ...
  G = pointBarycentric(tan1,tan2,tan3);                    // Gergonne-Punkt
...
function paint () {
  ...
  if (step >= 4) drawSegmentPP(A,T1,color1);               // 1. Verbindungsstrecke
  if (step >= 5) drawSegmentPP(B,T2,color1);               // 2. Verbindungsstrecke
  if (step >= 6) drawSegmentPP(C,T3,color1);               // 3. Verbindungsstrecke
  ...
  if (step >= 7) drawPoint(G,color2,gergonnepoint);        // Gergonne-Punkt
  }
