...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_steinercircumellipse_de.js) abgespeichert.
...
// d ...... Drehwinkel (Bogenmaß, Gegenuhrzeigersinn)
...
function drawCircumEllipseBCC (c1, c2, c3, col) {
  ...
  ctx.stroke();                                            // Polygonzug als Näherung für Ellipse
  }
...
function drawEllipseMPPP (M, P1, P2, P3, col) {
  ...
  var det = a11*a22*a33+a12*a23*a31+a13*a21*a32                 // Anfang Determinante (für Nenner)
    -a31*a22*a13-a32*a23*a11-a33*a21*a12;                       // Fortsetzung Determinante (für Nenner)
  var det1 = a22*a33+a12*a23+a13*a32-a22*a13-a32*a23-a33*a12;   // Zähler für Koeffizient a
  var det2 = a11*a33+a23*a31+a13*a21-a31*a13-a23*a11-a33*a21;   // Zähler für Koeffizient b
  var det3 = a11*a22+a12*a31+a21*a32-a31*a22-a32*a11-a21*a12;   // Zähler für Koeffizient c
  var a = det1/det, b = det2/det, c = det3/det;                 // Koeffizienten Ellipsengleichung
                                                                // Ursprung M; a*x^2 + 2*b*x*y + c*y^2 = 1
  var delta = Math.atan(2*b/(c-a))/2;                           // Drehwinkel (Bogenmaß, Uhrzeigersinn)
  var m = -Math.tan(delta);                                     // Steigung
  var aEll = semiaxis(a,b,c,m);                                 // 1. Halbachse (nicht unbedingt größer)
...
function paint () {
  ...
  if (step == 1) {                                         // Gegebenenfalls ...
    for (var i=1; i<=3; i++)                               // Für mehrere Werte des ersten Parameters ...
    for (var j=1; j<=3; j++)                               // Für mehrere Werte des zweiten Parameters ...
    for (var k=1; k<=3; k++)                               // Für mehrere Werte des dritten Parameters ...
