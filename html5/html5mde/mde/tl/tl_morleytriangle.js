// Dreiecks-Labor: Morley-Dreieck
// Java-Applet (28.10.2004) umgewandelt
// 17.02.2017 - 28.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_morleytriangle_de.js) abgespeichert.
...
var color4 = "#40ff40";                                    // Farbe für Morley-Dreieck
...
var step;                                                  // Einzelschritt (0 bis 11)
var M1, M2, M3;                                            // Ecken des Morley-Dreiecks
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, M1, M2, M3
...
function update () {
  ...
  var h1 = 2*Math.cos(alpha/3);                            // Hilfsgröße
  var h2 = 2*Math.cos(beta/3);                             // Hilfsgröße
  var h3 = 2*Math.cos(gamma/3);                            // Hilfsgröße
  M1 = pointBarycentric(a,b*h3,c*h2);                      // 1. Ecke des Morley-Dreiecks (in der Nähe von [BC])
  M2 = pointBarycentric(a*h3,b,c*h1);                      // 2. Ecke des Morley-Dreiecks (in der Nähe von [CA])
  M3 = pointBarycentric(a*h2,b*h1,c);                      // 3. Ecke des Morley-Dreiecks (in der Nähe von [AB])
  }
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, M1, M2, M3
...
function reactionNext () {
  ...
  if (step >= 11) bu2.disabled = true;                     // Falls nötig, Schaltknopf "Nächster Schritt" deaktivieren
...
function paint () {
  ...
  if (step >= 10) triangle(M1,M2,M3,color4);               // Morley-Dreieck (ausgefüllt)
  ...
  if (step >= 1) {                                         // Gegebenenfalls ...
    drawRayPP(A,M2,color1);                                // 1. Teilungslinie von alpha
    drawRayPP(A,M3,color1);                                // 2. Teilungslinie von alpha
    }
  if (step >= 2) {                                         // Gegebenenfalls ...
    drawRayPP(B,M3,color1);                                // 1. Teilungslinie von beta
    drawRayPP(B,M1,color1);                                // 2. Teilungslinie von beta
    }
  if (step >= 3) {                                         // Gegebenenfalls ...
    drawRayPP(C,M1,color1);                                // 1. Teilungslinie von gamma
    drawRayPP(C,M2,color1);                                // 2. Teilungslinie von gamma
    }
  if (step >= 7) drawSegmentPP(M2,M3,color2);              // 1. Seite Morley-Dreieck
  if (step >= 8) drawSegmentPP(M3,M1,color2);              // 2. Seite Morley-Dreieck
  if (step >= 9) drawSegmentPP(M1,M2,color2);              // 3. Seite Morley-Dreieck
  drawVertex(A,vertex1);                                   // Ecke A
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step >= 4 && step <= 9) drawPoint(M1,color2);        // 1. Ecke Morley-Dreieck
  if (step >= 5 && step <= 9) drawPoint(M2,color2);        // 2. Ecke Morley-Dreieck
  if (step >= 6 && step <= 9) drawPoint(M3,color2);        // 3. Ecke Morley-Dreieck
  }
