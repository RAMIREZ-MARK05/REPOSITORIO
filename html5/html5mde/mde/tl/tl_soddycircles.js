// Dreiecks-Labor: Soddy-Kreise
// 01.03.2017 - 07.09.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_soddycircles_de.js) abgespeichert.
...
var M1, M2;                                                // Mittelpunkte der Soddy-Kreise
var r1, r2;                                                // Radien der Soddy-Kreise
...
function start () {
  ...
  begin(110,260,230,260,120,160);                          // Anfangszustand
  paint();                                                 // Zeichnen
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, I, T1, T2, T3, rIC, M1, r1, M2, r2
...
function update () {
  ...
  T1 = pointBarycentric(0,tan2,tan3);                      // Berührpunkt auf [BC]
  T2 = pointBarycentric(tan1,0,tan3);                      // Berührpunkt auf [CA]
  T3 = pointBarycentric(tan1,tan2,0);                      // Berührpunkt auf [AB]
  var s = (a+b+c)/2;                                       // Halber Umfang
  var area = Math.sqrt(s*(s-a)*(s-b)*(s-c));               // Flächeninhalt (Heronformel)
  rIC = Math.sqrt((s-a)*(s-b)*(s-c)/s);                    // Inkreisradius
  var rCC = a*b*c/(4*area);                                // Umkreisradius
  var bcc1 = a+area/(s-a);                                 // 1. baryzentrische Koordinate
  var bcc2 = b+area/(s-b);                                 // 2. baryzentrische Koordinate
  var bcc3 = c+area/(s-c);                                 // 3. baryzentrische Koordinate
  M1 = pointBarycentric(bcc1,bcc2,bcc3);                   // Mittelpunkt des inneren Soddy-Kreises
  r1 = area/(4*rCC+rIC+2*s);                               // Radius des inneren Soddy-Kreises
  bcc1 = a-area/(s-a);                                     // 1. baryzentrische Koordinate
  bcc2 = b-area/(s-b);                                     // 2. baryzentrische Koordinate
  bcc3 = c-area/(s-c);                                     // 3. baryzentrische Koordinate
  M2 = pointBarycentric(bcc1,bcc2,bcc3);                   // Mittelpunkt des äußeren Soddy-Kreises
  r2 = Math.abs(area/(4*rCC+rIC-2*s));                     // Radius des äußeren Soddy-Kreises
  }
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, I, T1, T2, T3, rIC, M1, r1, M2, r2
...
function paint () {
  ...
  if (step >= 4) drawCircleMP(A,T2,color1);                // Kreis um A
  if (step >= 5) drawCircleMP(B,T3,color1);                // Kreis um B
  if (step >= 6) drawCircleMP(C,T1,color1);                // Kreis um C
  if (step >= 8) drawCircleMR(M1,r1,color2);               // Innerer Soddy-Kreis
  if (step >= 9) drawCircleMR(M2,r2,color2);               // Äußerer Soddy-Kreis
  drawVertex(A,vertex1);                                   // Ecke A
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step >= 1) drawPoint(T1,color1);                     // Berührpunkt auf [BC]
  if (step >= 2) drawPoint(T2,color1);                     // Berührpunkt auf [CA]
  if (step >= 3) drawPoint(T3,color1);                     // Berührpunkt auf [AB]
  if (step >= 8) drawPoint(M1,color2);                     // Mittelpunkt des inneren Soddy-Kreises
  if (step >= 9) drawPoint(M2,color2);                     // Mittelpunkt des äußeren Soddy-Kreises
  }
