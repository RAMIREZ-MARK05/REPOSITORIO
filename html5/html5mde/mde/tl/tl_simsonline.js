// Dreiecks-Labor: Simson-Gerade
// 01.05.2017 - 07.09.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_simsonline_de.js) abgespeichert.
...
var color3 = "#c0c0c0";                                    // Farbe für Versuche
...
var phi;                                                   // Positionswinkel (Bogenmaß)
var rCC;                                                   // Umkreisradius
var P;                                                     // Gegebener Punkt (auf dem Umkreis)
var F1, F2, F3;                                            // Fußpunkte der Lote
...
function start () {
  ...
  begin(100,300,300,300,150,120,250,50);                   // Anfangszustand
  paint();                                                 // Zeichnen
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, U, rCC, phi, F1, F2, F3
...
function update () {
  ...
  rCC = a*b*c/(4*area);                                    // Umkreisradius (Pixel)
  phi = Math.atan2(U.y-P.y,P.x-U.x);                       // Positionswinkel
  F1 = foot(P,B,C);                                        // Fußpunkt des ersten Lotes
  F2 = foot(P,C,A);                                        // Fußpunkt des zweiten Lotes
  F3 = foot(P,A,B);                                        // Fußpunkt des dritten Lotes
  }
...
// (px,py) ... Koordinaten des Punktes P
// Seiteneffekt step, A, B, C, P, nr, a, b, c, alpha, beta, gamma, U, rCC, phi, F1, F2, F3

function begin (ax, ay, bx, by, cx, cy, px, py) {
  step = 0;                                                // Einzelschritt
  A = {x: ax, y: ay};                                      // Ecke A
  B = {x: bx, y: by};                                      // Ecke B
  C = {x: cx, y: cy};                                      // Ecke C
  P = {x: px, y: py};                                      // Gegebener Punkt P
...
// Seiteneffekt step, P, a, b, c, alpha, beta, gamma, U, rCC, phi, F1, F2, F3

function reactionNext () {
  step++;                                                  // Nächster Einzelschritt
  if (step == 6) {                                         // Falls P auf Umkreis festgelegt ...
    P.x = U.x+rCC*Math.cos(phi);                           // x-Koordinate von P anpassen
    P.y = U.y-rCC*Math.sin(phi);                           // y-Koordinate von P anpassen
    update();                                              // Daten aktualisieren
    }
...
function reactionDown (u, v) {
  ...
  d2New = distance2(u,v,P);                                // Abstand zum Punkt P
  if (d2New < d2Min) {n = 4; d2Min = d2New;}               // Gegebenenfalls minimalen Abstand und Nummer aktualisieren
  nr = (d2Min < 400 ? n : 0);                              // Bei zu großem Abstand keine Ecke ausgewählt
  }
  
// Reaktion auf Bewegung von Maus oder Finger (Änderung):
// u, v ... Bildschirmkoordinaten bezüglich Viewport
// Seiteneffekt A, B, C, P, a, b, c, alpha, beta, gamma, U, rCC, phi, F1, F2, F3
...
function reactionMove (u, v) {
  ...
  if (nr >= 4 && step <= 5) {                              // Falls P frei verschiebbar ...
    P.x = u; P.y = v;                                      // Koordinaten von P anpassen
    } 
  if (nr == 4 && step >= 6) {                              // Falls P nur auf Umkreis verschiebbar ...
    phi = Math.atan2(U.y-v,u-U.x);                         // Positionswinkel
    P.x = U.x+rCC*Math.cos(phi);                           // x-Koordinate von P anpassen
    P.y = U.y-rCC*Math.sin(phi);                           // y-Koordinate von P anpassen
    }
  update();                                                // Daten aktualisieren
...
// Fußpunkt des Lotes von einem Punkt auf eine Gerade:
// p ...... Punkt
// a, b ... Bestimmungspunkte der Geraden

function foot (p, a, b) {
  var ux = b.x-a.x, uy = b.y-a.y;                          // Richtungsvektor AB
  var vx = p.x-a.x, vy = p.y-a.y;                          // Vektor AP
  var par = (ux*vx+uy*vy)/(ux*ux+uy*uy);                   // Parameterwert
  return {x: a.x+par*ux, y: a.y+par*uy};                   // Rückgabewert
  }
...
function paint () {
  ...
  if (step >= 1) drawSegmentPP(P,F1,color1);               // Erstes Lot
  if (step >= 2) drawSegmentPP(P,F2,color1);               // Zweites Lot
  if (step >= 3) drawSegmentPP(P,F3,color1);               // Drittes Lot
  if (step >= 5) drawCircleMR(U,rCC,color1);               // Umkreis
  drawVertex(A,vertex1);                                   // Ecke A
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  drawPoint(P,color0,givenPoint);                          // Gegebener Punkt
  if (step >= 1) drawPoint(F1,color1);                     // Fußpunkt des ersten Lotes
  if (step >= 2) drawPoint(F2,color1);                     // Fußpunkt des zweiten Lotes
  if (step >= 3) drawPoint(F3,color1);                     // Fußpunkt des dritten Lotes
  if (step >= 4 && step <= 5) {                            // Falls Versuchsstadium ...
    drawLinePP(F2,F3,color3);                              // Verbindungsgerade zweier Fußpunkte
    drawLinePP(F3,F1,color3);                              // Verbindungsgerade zweier Fußpunkte
    drawLinePP(F1,F2,color3);                              // Verbindungsgerade zweier Fußpunkte
    }
  if (step >= 6) drawLinePP(F1,F2,color2);                 // Simson-Gerade
  }
