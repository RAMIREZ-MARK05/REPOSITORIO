// Dreiecks-Labor: Euler-Gerade
// Java-Applet (28.10.2004) umgewandelt
// 22.01.2017 - 22.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_eulerline_de.js) abgespeichert.
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, M1, M2, M3, S, U, H, F1, F2, F3
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, M1, M2, M3, S, U, H, F1, F2, F3
...
function paint () {
  ...
  if (step == 1 || step >= 3) {                            // Gegebenenfalls ... 
  ...
  if (step >= 3) drawLinePP(U,H,color2);                   // Euler-Gerade
  drawVertex(A,vertex1);                                   // Ecke A
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step == 0 || step >= 3) drawPoint(S,color1,centroid);// Schwerpunkt S
  if (step == 1 || step >= 3) drawPoint(U,color1,circumcenter); // Umkreismittelpunkt U
  if (step >= 2) drawPoint(H,color1,orthocenter);          // Höhenschnittpunkt H
  }
