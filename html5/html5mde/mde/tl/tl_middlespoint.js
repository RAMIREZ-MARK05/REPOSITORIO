// Dreiecks-Labor: Mittenpunkt
// Java-Applet (28.10.2004) umgewandelt
// 23.01.2017 - 08.09.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_middlespoint_de.js) abgespeichert.
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, A1, A2, A3, r1, r2, r3, M1, M2, M3, M
...
function update () {
  ...
  M = pointBarycentric(a*(b+c-a),b*(c+a-b),c*(a+b-c));     // Mittenpunkt
  }
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, A1, A2, A3, r1, r2, r3, M1, M2, M3, M
...
function paint () {
  ...
  if (step >= 4) drawLinePP(A1,M1,color1);                 // 1. Verbindungsgerade
  if (step >= 5) drawLinePP(A2,M2,color1);                 // 2. Verbindungsgerade
  if (step >= 6) drawLinePP(A3,M3,color1);                 // 3. Verbindungsgerade
  ...
  if (step >= 1) drawPoint(M1,color1);                     // Mittelpunkt von [BC]
  if (step >= 2) drawPoint(M2,color1);                     // Mittelpunkt von [CA]
  if (step >= 3) drawPoint(M3,color1);                     // Mittelpunkt von [AB]
  if (step >= 7) drawPoint(M,color2,middlespoint);         // Mittenpunkt
  }
