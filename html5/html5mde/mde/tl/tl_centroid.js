// Dreiecks-Labor: Seitenhalbierende und Schwerpunkt
// Java-Applet (28.10.2004) umgewandelt
// 08.01.2017 - 21.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_centroid_de.js) abgespeichert.
...
// Seiteneffekt a, b, c, alpha, beta, gamma, S, M1, M2, M3
...
function paint () {
  ...
  if (step >= 4) drawSegmentPP(A,M1,color1);               // Seitenhalbierende s_a
  if (step >= 5) drawSegmentPP(B,M2,color1);               // Seitenhalbierende s_b
  if (step >= 6) drawSegmentPP(C,M3,color1);               // Seitenhalbierende s_c  
  drawVertex(A,vertex1);                                   // Ecke A 
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step >= 1 && step <= 3) drawPoint(M1,color1,midpoint1); // Mittelpunkt von Seite a (mit Bezeichnung)
  if (step >= 4) drawPoint(M1,color1);                     // Mittelpunkt von Seite a (ohne Bezeichnung)
  if (step >= 2 && step <= 3) drawPoint(M2,color1,midpoint2); // Mittelpunkt von Seite b (mit Bezeichnung)
  if (step >= 4) drawPoint(M2,color1);                     // Mittelpunkt von Seite b (ohne Bezeichnung)
  if (step == 3) drawPoint(M3,color1,midpoint3);           // Mittelpunkt von Seite c (mit Bezeichnung)
  if (step >= 4) drawPoint(M3,color1);                     // Mittelpunkt von Seite c (ohne Bezeichnung)
  if (step >= 8) drawPoint(S,color2,centroid);             // Schwerpunkt
  }
