// Dreiecks-Labor: Nagel-Punkt
// Java-Applet (28.10.2004) umgewandelt
// 22.01.2017 - 25.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_nagelpoint_de.js) abgespeichert.
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, r1, r2, r3, A1, A2, A3, T1, T2, T3
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, r1, r2, r3, A1, A2, A3, T1, T2, T3
...
function paint () {
  ...
  if (step >= 2) drawSegmentPP(A,T1,color1);               // 1. Verbindungsstrecke
  if (step >= 4) drawSegmentPP(B,T2,color1);               // 2. Verbindungsstrecke
  ...
  if (step >= 1) drawPoint(T1,color1);                     // Berührpunkt von Ankreis 1
  if (step >= 3) drawPoint(T2,color1);                     // Berührpunkt von Ankreis 2
  if (step >= 5) drawPoint(T3,color1);                     // Berührpunkt von Ankreis 3
  if (step >= 7) drawPoint(N,color2,nagelpoint);           // Nagel-Punkt
  }
