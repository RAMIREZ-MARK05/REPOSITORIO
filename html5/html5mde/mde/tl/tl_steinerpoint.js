// Dreiecks-Labor: Steiner-Kreise und Steiner-Punkt
// 02.12.2019 - 08.09.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_steinerpoint_de.js) abgespeichert.
...
var S;                                                     // Schwerpunkt
var A1, B1, C1;                                            // Spiegelpunkte der Ecken bezüglich Schwerpunkt
var k1, k2, k3;                                            // Steiner-Kreise
var St;                                                    // Steiner-Punkt
...
function start () {
  ...
  begin(80,280,300,280,140,110);                           // Anfangszustand
  paint();                                                 // Zeichnen
...
// Seiteneffekt a, b, c, M1, M2, M3, S, A1, B1, C1, k1, k2, k3, St
...
function update () {
  ...
  A1 = pointBarycentric(-1,2,2);                           // Spiegelpunkt von Ecke A bezüglich Schwerpunkt
  B1 = pointBarycentric(2,-1,2);                           // Spiegelpunkt von Ecke B bezüglich Schwerpunkt
  C1 = pointBarycentric(2,2,-1);                           // Spiegelpunkt von Ecke C bezüglich Schwerpunkt
  k1 = circumcircle(A,B1,C1);                              // Steiner-Kreis zu A
  k2 = circumcircle(A1,B,C1);                              // Steiner-Kreis zu B
  k3 = circumcircle(A1,B1,C);                              // Steiner-Kreis zu C
  var h1 = (c*c-a*a)*(a*a-b*b);                            // 1. baryzentrische Koordinate
  var h2 = (a*a-b*b)*(b*b-c*c);                            // 2. baryzentrische Koordinate
  var h3 = (b*b-c*c)*(c*c-a*a);                            // 3. baryzentrische Koordinate
  St = pointBarycentric(h1,h2,h3);                         // Steiner-Punkt
  }
...
// Seiteneffekt step, A, B, C, nr, a, b, c, M1, M2, M3, S, A1, B1, C1, k1, k2, k3, St
...
function paint () {
  ...
  if (step == 0) drawSegmentPP(A,M1,color1);               // Seitenhalbierende von a
  if (step == 0) drawSegmentPP(B,M2,color1);               // Seitenhalbierende von b
  if (step == 0) drawSegmentPP(C,M3,color1);               // Seitenhalbierende von c
  if (step == 4 || step >= 7) drawCircle(k1,color2);       // Steiner-Kreis zu A
  if (step == 5 || step >= 7) drawCircle(k2,color2);       // Steiner-Kreis zu B
  if (step == 6 || step >= 7) drawCircle(k3,color2);       // Steiner-Kreis zu C
  drawVertex(A,vertex1);                                   // Ecke A 
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  drawPoint(S,color1,centroid);                            // Schwerpunkt
  if (step >= 1) drawPoint(A1,color1,image1);              // Spiegelpunkt von A
  if (step >= 2) drawPoint(B1,color1,image2);              // Spiegelpunkt von B
  if (step >= 3) drawPoint(C1,color1,image3);              // Spiegelpunkt von C
  if (step >= 8) drawPoint(St,color2,steinerpoint);        // Steiner-Punkt
  }
