// Dreiecks-Labor: Malfatti-Kreise
// Java-Applet (10.11.2004) umgewandelt
// 03.02.2017 - 28.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_malfatticircles_de.js) abgespeichert.
...
var M1, M2, M3;                                            // Mittelpunkte der Malfatti-Kreise
var r1, r2, r3;                                            // Radien der Malfatti-Kreise
...
function start () {
  ...
  begin(50,300,350,300,100,100);                           // Anfangszustand
  paint();                                                 // Zeichnen
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, r1, r2, r3, M1, M2, M3 
...
function update () {
  ...
  var rIC = Math.sqrt((s-a)*(s-b)*(s-c)/s);                // Inkreisradius
  var rEC1 = Math.sqrt(s*(s-b)*(s-c)/(s-a));               // 1. Ankreisradius
  var rEC2 = Math.sqrt(s*(s-a)*(s-c)/(s-b));               // 2. Ankreisradius
  var rEC3 = Math.sqrt(s*(s-a)*(s-b)/(s-c));               // 3. Ankreisradius
  var ia = distancePP(I,A);                                // Abstand Inkreismittelpunkt - Ecke A
  var ib = distancePP(I,B);                                // Abstand Inkreismittelpunkt - Ecke B
  var ic = distancePP(I,C);                                // Abstand Inkreismittelpunkt - Ecke C
  r1 = rIC/(2*(s-a))*(s-rIC-(ib+ic-ia));                   // Radius des 1. Malfatti-Kreises
  r2 = rIC/(2*(s-b))*(s-rIC-(ic+ia-ib));                   // Radius des 2. Malfatti-Kreises
  r3 = rIC/(2*(s-c))*(s-rIC-(ia+ib-ic));                   // Radius des 3. Malfatti-Kreises
  var f = 2*rIC*s;                                         // Faktor für baryzentrische Koordinaten
  M1 = pointBarycentric(f*(1/r1-1/rEC1)-a,b,c);            // Mittelpunkt des 1. Malfatti-Kreises
  M2 = pointBarycentric(a,f*(1/r2-1/rEC2)-b,c);            // Mittelpunkt des 2. Malfatti-Kreises
  M3 = pointBarycentric(a,b,f*(1/r3-1/rEC3)-c);            // Mittelpunkt des 3. Malfatti-Kreises
  }
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, r1, r2, r3, M1, M2, M3 
...
function paint () {
  ...
  if (step >= 1) drawCircleMR(M1,r1,color2);               // 1. Malfatti-Kreis
  if (step >= 2) drawCircleMR(M2,r2,color2);               // 2. Malfatti-Kreis
  if (step >= 3) drawCircleMR(M3,r3,color2);               // 3. Malfatti-Kreis
  drawVertex(A,vertex1);                                   // Ecke A 
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step >= 1) drawPoint(M1,color2);                     // Mittelpunkt des 1. Malfatti-Kreises
  if (step >= 2) drawPoint(M2,color2);                     // Mittelpunkt des 2. Malfatti-Kreises
  if (step >= 3) drawPoint(M3,color2);                     // Mittelpunkt des 3. Malfatti-Kreises
  }
