// Dreiecks-Labor: Inkreis
// Java-Applet (28.10.2004) umgewandelt
// 15.01.2017 - 22.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_incircle_de.js) abgespeichert.
...
function update () {
  ...
  var tan1 = Math.tan(alpha/2);
  var tan2 = Math.tan(beta/2);
  var tan3 = Math.tan(gamma/2);
  T1 = pointBarycentric(0,tan2,tan3);
  T2 = pointBarycentric(tan1,0,tan3);
  T3 = pointBarycentric(tan1,tan2,0);
  var s = (a+b+c)/2;
  rIC = Math.sqrt((s-a)*(s-b)*(s-c)/s);
  }
...
function paint () {
  ...
  if (step == 3 || step == 6) drawAngle(B,A,C,"#80ffff");  // Winkel alpha
  if (step == 4 || step == 6) drawAngle(C,B,A,"#80ffff");  // Winkel beta
  if (step == 5 || step == 6) drawAngle(A,C,B,"#80ffff");  // Winkel gamma
  drawSegmentPP(B,C);                                      // Seite a
  drawSegmentPP(C,A);                                      // Seite b
  drawSegmentPP(A,B);                                      // Seite c
  if (step == 3 || step >= 6)                              // Gegebenenfalls ...
    drawRayPP(A,D,color1);                                 // Winkelhalbierende von alpha (Halbgerade)
  if (step == 4 || step >= 6)                              // Gegebenenfalls ... 
