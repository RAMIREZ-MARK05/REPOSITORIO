...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_bevanpoint_de.js) abgespeichert.
...
// Seiteneffekt a, b, c, alpha, beta, gamma, A1, A2, A3, r1, r2, r3, V, S, O, H, I, rIC
...
function update () {
  ...
  var cos1 = Math.cos(alpha);                              // Kosinus von alpha
  var cos2 = Math.cos(beta);                               // Kosinus von beta
  var cos3 = Math.cos(gamma);                              // Kosinus von gamma
  var v1 = a*(cos2+cos3-cos1-1);                           // 1. baryzentrische Koordinate
  var v2 = b*(cos3+cos1-cos2-1);                           // 2. baryzentrische Koordinate
  var v3 = c*(cos1+cos2-cos3-1);                           // 3. baryzentrische Koordinate
  V = pointBarycentric(v1,v2,v3);                          // Bevan-Punkt
  ...
  O = pointBarycentric(sin1,sin2,sin3);                    // Umkreismittelpunkt
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, A1, A2, A3, r1, r2, r3, V, S, O, H, I, rIC
...
// Lot zeichnen:
// p1, p2 ... Bestimmungspunkte der Geraden
// p ........ Punkt, durch den das Lot gefällt wird
// c ........ Farbe (optional, Defaultwert schwarz)

function drawPerpendicularPPP (p, p1, p2, c) {
  var ux = p2.x-p1.x, uy = p2.y-p1.y;                      // Richtungsvektor
  var vx = p.x-p1.x, vy = p.y-p1.y;                        // Verbindungsvektor
  var par = (ux*vx+uy*vy)/(ux*ux+uy*uy);                   // Parameterwert Fußpunkt
  var f = {x: p1.x+par*ux, y: p1.y+par*uy};                // Fußpunkt
  drawSegmentPP(p,f,c);                                    // Lot zeichnen
  }
...
function paint () {
  ...
  if (step >= 0) drawCircleMR(A1,r1,color1);               // Ankreis 1
  if (step >= 0) drawCircleMR(A2,r2,color1);               // Ankreis 2
  if (step >= 0) drawCircleMR(A3,r3,color1);               // Ankreis 3
  if (step >= 1) drawCircleMP(V,A1,color2);                // Kreis durch die Ankreismittelpunkte
  if (step >= 5) drawCircleMP(O,A,color1);                 // Umkreis
  if (step >= 5) drawCircleMR(I,rIC,color1);               // Inkreis
  if (step == 3 || step == 4) drawLinePP(A1,V,color2);     // Gerade durch 1. Ankreismittelpunkt und Bevan-Punkt
  if (step == 3 || step == 4) drawLinePP(A2,V,color2);     // Gerade durch 2. Ankreismittelpunkt und Bevan-Punkt
  if (step == 3 || step == 4) drawLinePP(A3,V,color2);     // Gerade durch 3. Ankreismittelpunkt und Bevan-Punkt
  if (step == 6) drawSegmentPP(I,V,color2);                // Verbindungsstrecke zwischen Inkreismittelpunkt und Bevan-Punkt
  if (step >= 7) drawLinePP(H,O,color1);                   // Euler-Gerade
  if (step >= 8) drawPerpendicularPPP(I,H,O,color2);       // Abstand Inkreismittelpunkt/Euler-Gerade
  if (step >= 8) drawPerpendicularPPP(V,H,O,color2);       // Abstand Bevan-Punkt/Euler-Gerade
  drawVertex(A,vertex1);                                   // Ecke A
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step >= 0) drawPoint(A1,color1);                     // Mittelpunkt von Ankreis 1
  if (step >= 0) drawPoint(A2,color1);                     // Mittelpunkt von Ankreis 2
  if (step >= 0) drawPoint(A3,color1);                     // Mittelpunkt von Ankreis 3
  if (step >= 2) drawPoint(V,color2,bevanpoint);           // Bevan-Punkt
  if (step >= 5) drawPoint(O,color1);                      // Umkreismittelpunkt
  if (step >= 5) drawPoint(I,color1);                      // Inkreismittelpunkt
  if (step >= 7) drawPoint(H,color1);                      // Höhenschnittpunkt
  if (step >= 7) drawPoint(S,color1);                      // Schwerpunkt
  }
