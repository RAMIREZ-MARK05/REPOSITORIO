...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_ninepointcircle_de.js) abgespeichert.
...
function paint () {
  ...
  if (step <= 9 || step >= 12) drawLinePP(A,F1,color1);    // Höhe h_a (verlängert)
  if (step <= 9 || step >= 12) drawLinePP(B,F2,color1);    // Höhe h_b (verlängert)
  if (step <= 9 || step >= 12) drawLinePP(C,F3,color1);    // Höhe h_c (verlängert)
  ...
  if (step <= 9 || step >= 12) drawPoint(H,color1,orthocenter); // Höhenschnittpunkt H
  if (step >= 12) drawPoint(S,color1,centroid);            // Schwerpunkt
  if (step >= 12) drawPoint(U,color1,circumcenter);        // Umkreismittelpunkt
  if (step >= 1) drawPoint(F1,color1);                     // Fußpunkt auf BC
  if (step >= 2) drawPoint(F2,color1);                     // Fußpunkt auf CA
  if (step >= 3) drawPoint(F3,color1);                     // Fußpunkt auf AB
  if (step >= 4) drawPoint(M1,color1);                     // Mittelpunkt von Höhenabschnitt h_a
  if (step >= 5) drawPoint(M2,color1);                     // Mittelpunkt von Höhenabschnitt h_b
  if (step >= 6) drawPoint(M3,color1);                     // Mittelpunkt von Höhenabschnitt h_c
