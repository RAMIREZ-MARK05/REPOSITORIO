// Dreiecks-Labor: Südpolsatz
// Java-Applet (28.10.2004) umgewandelt
// 23.01.2017 - 07.09.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_circumcircle_de.js) abgespeichert.
...
var color3 = "#80ffff";                                    // Farbe für Winkelmarkierungen
...
// Seiteneffekt a, b, c, alpha, beta, gamma, M1, M2, M3, U, D, E, F, SP1, SP2, SP3
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, M1, M2, M3, U, D, E, F, SP1, SP2, SP3
...
function paint () {
  ...
  if (step >= 4) drawAngle(B,A,C,color3);                  // Winkel alpha
  if (step >= 6) drawAngle(C,B,A,color3);                  // Winkel beta
  if (step >= 8) drawAngle(A,C,B,color3);                  // Winkel gamma
  drawSegmentPP(B,C);                                      // Seite a
  drawSegmentPP(C,A);                                      // Seite b
  drawSegmentPP(A,B);                                      // Seite c
  if (step >= 1) drawPerpendicularLine(B,C,M1,color1);     // Mittelsenkrechte zu [BC]
  if (step >= 2) drawPerpendicularLine(C,A,M2,color1);     // Mittelsenkrechte zu [CA]
  if (step >= 3) drawPerpendicularLine(A,B,M3,color1);     // Mittelsenkrechte zu [AB]
  if (step >= 5) drawRayPP(A,D,color1);                    // Winkelhalbierende von alpha
  if (step >= 7) drawRayPP(B,E,color1);                    // Winkelhalbierende von beta
  if (step >= 9) drawRayPP(C,F,color1);                    // Winkelhalbierende von gamma
  drawCircleMP(U,A,color1);                                // Umkreis
  drawVertex(A,vertex1);                                   // Ecke A
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step >= 10) drawPoint(SP1,color2);                   // Südpol 1
  if (step >= 11) drawPoint(SP2,color2);                   // Südpol 2
  if (step >= 12) drawPoint(SP3,color2);                   // Südpol 3
  }
