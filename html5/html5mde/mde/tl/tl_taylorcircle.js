...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_taylorcircle_de.js) abgespeichert.
...
var X12, X13, X21, X23, X31, X32;                          // Fußpunkte der Nebenhöhen
...
function update () {
  ...
  X12 = pointBarycentric(tan1*tan2,0,tan3*sum);            // Fußpunkt des Lotes von F1 auf AC
  X13 = pointBarycentric(tan1*tan3,tan2*sum,0);            // Fußpunkt des Lotes von F1 auf AB
  X23 = pointBarycentric(tan1*sum,tan2*tan3,0);            // Fußpunkt des Lotes von F2 auf BA
  X21 = pointBarycentric(0,tan1*tan2,tan3*sum);            // Fußpunkt des Lotes von F2 auf BC
  X31 = pointBarycentric(0,tan2*sum,tan1*tan3);            // Fußpunkt des Lotes von F3 auf CB
  X32 = pointBarycentric(tan1*sum,0,tan2*tan3);            // Fußpunkt des Lotes von F3 auf CA
...
function reactionNext () {
  ...
  if (step >= 17) bu2.disabled = true;                     // Falls nötig, Schaltknopf "Nächster Schritt" deaktivieren
...
function paint () {
  ...
  drawLinePP(A,F1,color1);                                 // Höhe h_a (verlängert)
  drawLinePP(B,F2,color1);                                 // Höhe h_b (verlängert)
  drawLinePP(C,F3,color1);                                 // Höhe h_c (verlängert)  
  ...
  if (step >= 1 && step <= 3) drawPoint(F1,color1);        // Fußpunkt der Höhe h_a
  if (step >= 4 && step <= 6) drawPoint(F2,color1);        // Fußpunkt der Höhe h_b
  if (step >= 7 && step <= 9) drawPoint(F3,color1);        // Fußpunkt der Höhe h_c
  if (step >= 11) drawPoint(X23,color1);                   // 1. Nebenhöhenfußpunkt
  if (step >= 12) drawPoint(X13,color1);                   // 2. Nebenhöhenfußpunkt
  if (step >= 13) drawPoint(X31,color1);                   // 3. Nebenhöhenfußpunkt
  if (step >= 14) drawPoint(X21,color1);                   // 4. Nebenhöhenfußpunkt
  if (step >= 15) drawPoint(X12,color1);                   // 5. Nebenhöhenfußpunkt
  if (step >= 16) drawPoint(X32,color1);                   // 6. Nebenhöhenfußpunkt
