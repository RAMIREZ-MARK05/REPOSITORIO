// Dreiecks-Labor: Fermat-Punkte
// Java-Applet (28.10.2004) umgewandelt
// 03.02.2017 - 27.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_fermatpoints_de.js) abgespeichert.
...
function start () {
  ...
  ch = getElement("ch");                                   // Auswahlfeld (1./2. Fermat-Punkt)
  ...
  sign = 1;                                                // Vorzeichenfaktor für 1. Fermat-Punkt
  ...
  ch.onchange = reactionSelect;                            // Reaktion auf Auswahlfeld (1./2. Fermat-Punkt)
...
// Daten aktualisieren:
// Seiteneffekt a, b, c, alpha, beta, gamma, N1, N2, N3, F
...
function update () {
  ...
  var w1 = alpha+sign*Math.PI/3;                           // Winkel für baryzentrische Koordinaten
  var w2 = beta+sign*Math.PI/3;                            // Winkel für baryzentrische Koordinaten
  var w3 = gamma+sign*Math.PI/3;                           // Winkel für baryzentrische Koordinaten
  F = pointBarycentric(a/Math.sin(w1),b/Math.sin(w2),c/Math.sin(w3));     // 1. oder 2. Fermat-Punkt
  }
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, N1, N2, N3, F
...
// Reaktion auf Auswahlfeld (1./2. Fermat-Punkt):
...
function paint () {
  ...
  if (step >= 5) drawLinePP(A,N1,color1);                  // 1. Verbindungsgerade
  if (step >= 7) drawLinePP(B,N2,color1);                  // 2. Verbindungsgerade
  if (step >= 9) drawLinePP(C,N3,color1);                  // 3. Verbindungsgerade
  drawVertex(A,vertex1);                                   // Ecke A 
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  if (step >= 4) drawPoint(N1,color1);                     // Neue Ecke des 1. aufgesetzten Dreiecks
  if (step >= 6) drawPoint(N2,color1);                     // Neue Ecke des 2. aufgesetzten Dreiecks
  if (step >= 8) drawPoint(N3,color1);                     // Neue Ecke des 3. aufgesetzten Dreiecks
  var fp = (sign>0 ? fermatpoint1 : fermatpoint2);         // Bezeichnung für Fermat-Punkt (1. oder 2.)
  if (step >= 10) drawPoint(F,color2,fp);                  // Fermat-Punkt (1. oder 2.)
  }
