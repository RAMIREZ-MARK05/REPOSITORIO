// Dreiecks-Labor: Höhenfußpunktdreieck
// Java-Applet (28.10.2004) umgewandelt
// 01.02.2017 - 29.08.2022
...
// Sprachabhängige Texte sind einer eigenen Datei (zum Beispiel tl_orthictriangle_de.js) abgespeichert.
...
// Seiteneffekt step, A, B, C, nr, a, b, c, alpha, beta, gamma, H, F1, F2, F3
...
function paint () {
  ...
  if (step >= 4) drawSegmentPP(F2,F3,color2);              // 1. Seite des Fußpunktdreiecks
  if (step >= 5) drawSegmentPP(F3,F1,color2);              // 2. Seite des Fußpunktdreiecks
  if (step >= 6) drawSegmentPP(F1,F2,color2);              // 3. Seite des Fußpunktdreiecks  
  drawVertex(A,vertex1);                                   // Ecke A 
  drawVertex(B,vertex2);                                   // Ecke B
  drawVertex(C,vertex3);                                   // Ecke C
  drawPoint(H,color1,orthocenter);                         // Höhenschnittpunkt H
  if (step >= 1) drawPoint(F1,color1);                     // Fußpunkt der Höhe h_a
  if (step >= 2) drawPoint(F2,color1);                     // Fußpunkt der Höhe h_b
  if (step >= 3) drawPoint(F3,color1);                     // Fußpunkt der Höhe h_c
  }
